...
const resolveImageForViewer = async (cdnUrl: string): Promise<string> => {
  if (objectUrlCache.has(cdnUrl)) {
    return objectUrlCache.get(cdnUrl)!;
  }
  try {
...
const loadAll = async (): Promise<{ profileUrl: string; profile: StoredProfile }[]> => {
...
document.getElementById("clear-btn")!.addEventListener("click", async () => {
  if (!confirm("Clear all cached profile data?")) {
    return;
  }
